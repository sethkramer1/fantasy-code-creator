...
export function Terminal({ output, thinkingTime, generationInProgress }: TerminalProps) {
  ...
  // Process output to ensure it's an array of strings
  const outputArray = typeof output === 'string' ? output.split('\n') : output;
  ...
  useEffect(() => {
    ...
    const shouldAutoScroll = outputArray.length > previousOutputLength.current;
    previousOutputLength.current = outputArray.length;
  ...
  }, [outputArray]); // React to output changes

  // Process output lines to better format code blocks
  const processedOutput = outputArray.map(line => {
